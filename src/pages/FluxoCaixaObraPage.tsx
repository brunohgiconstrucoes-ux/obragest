...
import type { FluxoCaixa, Obra } from '@/types'
...
export function FluxoCaixaObraPage() {
  ...
  const totalSaidasRealizadas = lancamentos
    .filter(l => l.tipo === 'saida' && l.status === 'realizado')
    .reduce((s, l) => s + l.valor, 0)

  const margemReal = totalEntradasRealizadas - totalSaidasRealizadas
  ...
  return (
    <div className="text-[var(--color-text)] space-y-6">
      ...
      <div className="flex items-center gap-3">
        <Link to={`/obras/${id}`} className="text-[var(--color-muted)] hover:text-[var(--color-text)]">
          <ArrowLeft size={20} />
        </Link>
        <div>
          <h1 className="text-2xl font-bold">Fluxo de Caixa da Obra</h1>
          {obra && <p className="text-[var(--color-muted)] text-sm mt-0.5">{obra.nome}</p>}
        </div>
      </div>
      ...
      <div className="grid grid-cols-3 gap-4">
        <div className="bg-[var(--color-surface)] border border-[var(--color-border)] rounded-lg p-4">
          <p className="text-[var(--color-muted)] text-xs mb-1">Total Entradas Realizadas</p>
          <ValorMonetario value={totalEntradasRealizadas} className="text-[var(--color-success)] text-lg font-bold" />
        </div>
        <div className="bg-[var(--color-surface)] border border-[var(--color-border)] rounded-lg p-4">
          <p className="text-[var(--color-muted)] text-xs mb-1">Total Saídas Realizadas</p>
          <ValorMonetario value={totalSaidasRealizadas} className="text-[var(--color-danger)] text-lg font-bold" />
        </div>
        <div className="bg-[var(--color-surface)] border border-[var(--color-border)] rounded-lg p-4">
          <p className="text-[var(--color-muted)] text-xs mb-1">Margem Real</p>
          <ValorMonetario
            value={margemReal}
            className={`text-lg font-bold ${margemReal >= 0 ? 'text-[var(--color-success)]' : 'text-[var(--color-danger)]'}`}
          />
        </div>
      </div>
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="bg-[var(--color-surface)] border-[var(--color-border)] text-[var(--color-text)]">
          <DialogHeader>
            <DialogTitle className="text-[var(--color-text)]">Novo Lançamento Manual</DialogTitle>
          </DialogHeader>
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
            ...
            <div>
              ...
              <Input
                {...register('categoria')}
                className="mt-1 bg-[var(--color-surface-2)] border-[var(--color-border)] text-[var(--color-text)]"
                placeholder="Ex: Serviços, Insumos..."
              />
            </div>
            <div>
